...
// A dedicated GPX fixture with a genuine recognised climb (2000 m of
// nominal 8% grade -> category-4 once GPX-import smoothing/resampling has
// applied — verified directly against this exact fixture's own rendered
// dropdown option, not hand-computed), unlike trackWithElevationGpx above
// (4 points, far under both the 500 m length and 3% average-gradient
// recognised-climb thresholds) — the climb-score hand-off describe block
// below needs a route the pre-ride climb selector actually offers
// something to select. Coordinates are spaced by real geodesic distance
// (matching RidingScreen.test.tsx's own twoClimbRoute convention), since
// App-level import goes through genuine GPX distance computation rather
// than a hand-supplied distanceFromStartMetres.
const CLIMB_LON_PER_METRE = 1 / (111_320 * Math.cos((51.5 * Math.PI) / 180));
function buildClimbTrackGpx(): string {
  const points = Array.from({ length: 21 }, (_, index) => {
    const distanceMetres = index * 100;
    const lon = distanceMetres * CLIMB_LON_PER_METRE;
    const elevationMetres = distanceMetres * 0.08;
    return `      <trkpt lat="51.5000" lon="${lon.toFixed(8)}"><ele>${elevationMetres.toFixed(1)}</ele></trkpt>`;
  }).join("\n");
  return `<?xml version="1.0" encoding="UTF-8"?>
<gpx version="1.1" creator="acn-test-fixtures" xmlns="http://www.topografix.com/GPX/1/1">
  <trk>
    <name>Test climb track</name>
    <trkseg>
${points}
    </trkseg>
  </trk>
...
describe("App — climb-score explanation hand-off (item 78)", () => {
  beforeEach(async () => {
    await db.routes.clear();
    await db.rideState.clear();
    await db.routeLibraryPreferences.clear();
  ...
  async function importClimbFixture(
    user: ReturnType<typeof userEvent.setup>,
    name: string,
  ) {
    const file = buildGpxFile(name, buildClimbTrackGpx());
    const expectedName = name.replace(/\.gpx$/i, "");
    await user.upload(screen.getByLabelText("Import GPX file"), file);
    await waitFor(() => {
      expect(screen.getByRole("button", { name: expectedName })).toBeInTheDocument();
  ...
  async function selectTheClimb(user: ReturnType<typeof userEvent.setup>) {
    await user.selectOptions(
      screen.getByRole("combobox", { name: "Recognised climbs" }),
      "climb-0",
    );
    expect(
      await screen.findByRole("heading", { name: "Climb 1 · Category 4" }),
    ).toBeInTheDocument();
  }

  it("opens and focuses the Settings climb-classification explanation, then restores the same route and climb selection on return", async () => {
    const user = userEvent.setup();
    render(<App mapFactory={buildNoopMapFactory()} />);

    await importClimbFixture(user, "Climb Route.gpx");
    await user.click(screen.getByRole("button", { name: "Climb Route" }));
    await selectTheClimb(user);
    ...
    expect(screen.getByRole("heading", { name: "Climb Route" })).toBeInTheDocument();
  ...
  it("does not start GPS or mutate the persisted active-ride record merely by navigating to and from the explanation", async () => {
    const user = userEvent.setup();
    render(<App mapFactory={buildNoopMapFactory()} />);
  ...
  it("does not silently reapply a stale selection on a later, unrelated remount after the rider explicitly clears it", async () => {
    const user = userEvent.setup();
    render(<App mapFactory={buildNoopMapFactory()} />);
    ...
    // An unrelated round trip through a different screen must not resurrect
    // the old preservedFeatureId hand-off — RidingScreen.tsx clears it via
    // onFeatureSelectionHandoffConsumed the moment it was first used above.
    await user.click(screen.getByRole("button", { name: "Diagnostics" }));
    await user.click(screen.getByRole("button", { name: "Ride" }));
  ...
  it("repeated activation works a second time after a full round trip", async () => {
    const user = userEvent.setup();
    render(<App mapFactory={buildNoopMapFactory()} />);
  ...
  it("opening a different route while a hand-off is pending does not error, and the new route never inherits the old selection", async () => {
    const user = userEvent.setup();
    render(<App mapFactory={buildNoopMapFactory()} />);

    await importClimbFixture(user, "Climb Route.gpx");
    await importFixture(user, "Plain Route.gpx");
    await user.click(screen.getByRole("button", { name: "Climb Route" }));
    await selectTheClimb(user);
    await user.click(screen.getByRole("button", { name: "How is this calculated?" }));

    // Instead of returning to Ride, open a different route entirely.
    await user.click(screen.getByRole("button", { name: "Routes" }));
    await user.click(await screen.findByRole("button", { name: "Plain Route" }));

    expect(screen.getByRole("heading", { name: "Plain Route" })).toBeInTheDocument();
    expect(screen.queryByRole("heading", { name: /Climb 1/ })).toBeNull();

    // The Settings token was already consumed on its first appearance
    // above — a later, ordinary Settings visit must not reopen/refocus it.
    await user.click(screen.getByRole("button", { name: "Settings" }));
    const details = screen.getByText("How climbs are classified").closest("details");
