...
Stable item numbers never change regardless of which file an item's text lives in. Item 96 is selected as the next implementation item. The current suggested order for the remaining newly added items 96–103 is `96 → 97 → 98 → 99 → 100 → 101 → 102 → 103` — this is the current suggested order, not an immutable dependency chain.
...
| 96   | Grace period for the slow map-imagery notice                                                  | Pending — selected next                          | [`backlog.md#item-96`](docs/project/backlog.md#item-96)               |
