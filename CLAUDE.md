...
Stable item numbers never change regardless of which file an item's text lives in. Item 100 is **implementation-complete**: stages 1–3 (data model and storage; tag editing and reusable suggestions; tag filtering and organisation) and stage 4A (the global rename/merge/delete tag lifecycle plus empty-state coverage) have all shipped, so its full record now lives in [`docs/project/history/items-100-NN.md`](docs/project/history/items-100-NN.md#item-100). What remains is **stage 4B — physical-device acceptance only**, an open field-acceptance follow-up tracked in [`docs/project/current-status.md`](docs/project/current-status.md); no further item-100 coding is scheduled. Stage 4B's field test duly turned up two reveal-scrolling observations, and they became their own evidence-led follow-up, **item 105**, rather than reopening item 100. Item 105 was then deliberately reprioritised ahead of item 101 so that item 100 could be concluded, and is now **implemented in `0.4.20` and awaiting its own installed-iPhone acceptance**, which is the sole remaining gate on stage 4B; its full record lives in [`docs/project/history/items-100-NN.md`](docs/project/history/items-100-NN.md#item-105). Note that item 105's second behaviour was corrected before implementation: the field report's "reveal the Merge/Delete confirmation" wording mis-recorded the request, and what shipped instead reveals the **Manage tags panel's own top after a successful global operation**, leaving the confirmation's arming and cancelling behaviour untouched — see the item's own dated scope-correction bullet. **Item 101 is the next implementation slice.** The current suggested order for the remaining items is `101 → 102 → 103` — a suggested order, not an immutable dependency chain.
...
| 101  | Plain-language HTTP-status guidance in Routing diagnostics                                    | Pending — selected next                          | [`backlog.md#item-101`](docs/project/backlog.md#item-101)             |
...
Items 6–10, 13–15, 17–27, 29–31, 33–42, 44–58, 62–65, 67–99, 100 and 105 are completed (`— done`) and live in [`docs/project/history/`](docs/project/history/README.md). Item 100's own entry is code-complete only: its stage 4B physical acceptance remains open in [`docs/project/current-status.md`](docs/project/current-status.md). Item 43 is a follow-up acceptance checklist and lives in [`docs/project/current-status.md`](docs/project/current-status.md). Item 86 is the release-readiness audit that produced items 87–92; its full report is [`docs/project/release-readiness-audit.md`](docs/project/release-readiness-audit.md). Item 104 is also completed (`— done`, in [`docs/project/history/items-100-NN.md`](docs/project/history/items-100-NN.md)) — a route-progress correctness prerequisite implemented ahead of item 98, whose own higher number does not renumber or complete items 98–103, of which 101–103 remain pending in the order above. Item 105 is newer than item 104 and, like it, was completed ahead of items 101–103, which remain pending: a completed item's number never implies that lower-numbered work is finished, nor the reverse. Item 105's own entry is code-complete only — its installed-iPhone acceptance, and with it item 100 stage 4B's closure, remains open in [`docs/project/current-status.md`](docs/project/current-status.md). Item 104 comprises two changes: the original `0.4.8` correction and a dated follow-up in the same entry, which fixed a walking-cadence regression the first attempt missed and which failed its first physical acceptance; read both before relying on route progress. The principal installed-iPhone walking-cadence scenario is now accepted on deployed `0.4.9`; the remaining subchecks (a deliberate beyond-turn overshoot and its warning clearing, pause/reload/resume near the turnaround, a bicycle-speed run, and physical Android) stay open in [`docs/project/current-status.md`](docs/project/current-status.md), which remains the only acceptance ledger.
