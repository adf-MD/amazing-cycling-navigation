...
Stable item numbers never change regardless of which file an item's text lives in. Item 99 is selected as the next implementation item. The current suggested order for the remaining newly added items 99–103 is `99 → 100 → 101 → 102 → 103` — this is the current suggested order, not an immutable dependency chain.
...
| 99   | Route Library sorting by distance and total ascent                                            | Pending — selected next                          | [`backlog.md#item-99`](docs/project/backlog.md#item-99)               |
