...
9. **Climb detection and summary segments**
   - Gradient colouring itself is implemented (see Milestone 4's third slice above: `src/navigation/gradient.ts`, `gradientPalette.ts`, `src/map/gradientRouteLayer.ts`) — noise-resistant analysis, accessible non-colour legend cues, and shared classification between the elevation chart and the map are done.
   - Still outstanding: grouping gradient segments into named climbs and surfacing upcoming climb distance, length, ascent and average gradient as their own summary (a climb "card" or similar), reusing `GradientSegment` output rather than re-analysing.
