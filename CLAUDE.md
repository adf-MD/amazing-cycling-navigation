...
Stable item numbers never change regardless of which file an item's text lives in. Item 95 is selected as the next implementation item. The current suggested order for the remaining newly added items 95–103 is `95 → 96 → 97 → 98 → 99 → 100 → 101 → 102 → 103` — this is the current suggested order, not an immutable dependency chain.
...
| 95   | Route-switch prompt visibility and action hierarchy                                           | Pending — selected next                          | [`backlog.md#item-95`](docs/project/backlog.md#item-95)               |
