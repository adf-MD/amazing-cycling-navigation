...
Stable item numbers never change regardless of which file an item's text lives in. Item 100 stage 1 (data model and storage) is done; item 100 stage 2 (tag editing and reusable suggestions) is selected as the next implementation slice, ahead of item 101. The current suggested order for the remaining newly added items 100–103 is `100 → 101 → 102 → 103` — this is the current suggested order, not an immutable dependency chain.
...
| 100  | Reusable route tags and tag-based organisation                                                | In progress — stage 1 done; stage 2 selected next | [`backlog.md#item-100`](docs/project/backlog.md#item-100)             |
