...
98. **Direction-aware active-Riding route-segment layering**
    ...
    - Item 104 (`docs/project/history/items-95-NN.md#item-104`) is this item's completed route-progress prerequisite — but read it **together with its dated 2026-09-04 follow-up**, not as the `0.4.8` change alone. `0.4.8` corrected `src/navigation/projection.ts`'s windowed matching so canonical progress can advance onto the return leg of an exactly overlapping out-and-back turnaround; it then failed its first physical acceptance at walking cadence, and the follow-up (`0.4.9`) added the tied sub-epsilon hold in `src/navigation/rideNavigationCore.ts` that makes progress genuinely non-regressing there. The two together are the upstream automated correction, and the principal installed-iPhone walking-cadence scenario is now accepted on deployed `0.4.9` — see `docs/project/current-status.md`, which also records the subchecks that remain open (a deliberate beyond-turn overshoot and its warning clearing, pause/reload/resume near the turnaround, a bicycle-speed run, and physical Android). This item must consume that corrected progress as-is — do not change route projection/matching again as part of implementing this item.
    - Physical pre-fix evidence (deployed `0.4.9`, installed iPhone PWA, 2026-09-07): on an exactly overlapping out-and-back return leg the grey **completed outbound** trace is visibly painted above the green **remaining return** trace immediately ahead of the rider. That route reports `0 m ascent`, so this is not evidence for the climb-versus-descent palette case; it is direct evidence that the existing completed/remaining paint order alone cannot represent the rider's current direction at an exact geographic overlap.
