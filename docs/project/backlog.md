...
98. **Direction-aware active-Riding route-segment layering**
    ...
    - Item 104 (`docs/project/history/items-95-NN.md#item-104`) is this item's completed route-progress prerequisite: it corrected `src/navigation/projection.ts`'s windowed matching so canonical progress reliably advances onto the return leg of an exactly overlapping out-and-back turnaround, rather than the matched distance itself running backward. This item must consume that corrected progress as-is — do not change route projection/matching again as part of implementing this item.
